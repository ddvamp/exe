...
public:
    ...
    void Add(State const delta = 1) noexcept {
        [[maybe_unused]] auto const state =
            state_.fetch_add(delta, ::std::memory_order_relaxed);
        UTILS_ASSERT(IsAddCorrect(state, delta),
    ...
    void Done(State const delta = 1) {
        ...
        UTILS_ASSERT(IsDoneCorrect(state, delta),
...
private:
    ...
    [[nodiscard]] inline static bool IsNotifyNeeded(State const state)
        noexcept {
        return ::utils::all_of(GetCount(state) == 0,
            GetWaitersCount(state) != 0);
    }

    [[nodiscard]] inline static ::std::uint32_t GetCount(
        State const state) noexcept {
        return static_cast<::std::uint32_t>(state);
    }

    [[nodiscard]] inline static ::std::uint32_t GetWaitersCount(
        State const state) noexcept {
